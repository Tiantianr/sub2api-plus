...
// HasOpenaiOauthAccountGrants applies the HasEdge predicate on the "openai_oauth_account_grants" edge.
func HasOpenaiOauthAccountGrants() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasOpenaiOauthAccountGrantsWith applies the HasEdge predicate on the "openai_oauth_account_grants" edge with a given conditions (other predicates).
func HasOpenaiOauthAccountGrantsWith(preds ...predicate.OpenAIOAuthAccountUserGrant) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newOpenaiOauthAccountGrantsStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
