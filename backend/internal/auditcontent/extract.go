...
func appendResponsesPromptOrLegacyAlias(document *Document, value any, hasNativeInput bool) {
...
func extractAlphaSearch(document *Document, root map[string]any) {
	// PAT fallback serializes all three fields into an upstream user prompt.
	// Audit the same client-controlled JSON before account selection decides
	// whether the native alpha endpoint or that fallback is used.
...
// StructuredText returns the exact sanitized representation used by canonical
// structured audit segments. Callers that transform a payload into upstream
// prompt text can reuse it so stripped opaque/media fields are not forwarded
// without audit.
