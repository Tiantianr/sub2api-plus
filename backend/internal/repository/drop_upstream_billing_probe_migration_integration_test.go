...
func TestMigration234DropsProbeMetadataWithoutChangingManualRate(t *testing.T) {
	...
	migrationSQL, err := dbmigrations.FS.ReadFile("237_drop_upstream_billing_probe.sql")
...
`, accountID).Scan(&rate, &extra))
	...
	require.JSONEq(t, `{"codex_fingerprint_mode":"off","openai_long_context_billing_enabled":false,"retained":"value"}`, extra)
