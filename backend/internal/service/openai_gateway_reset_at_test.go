...
func TestFinalizeCodexClientQuotaHeadersPolicy(t *testing.T) {
	...
	passthroughAccounts := []*Account{
		{Platform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: map[string]any{"openai_passthrough": true}},
		{Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Extra: map[string]any{"openai_passthrough": true}},
	}
...
func TestFinalizeCodexClientQuotaEventPolicy(t *testing.T) {
	...
	require.Equal(t, "codex", gjson.GetBytes(local, "metered_limit_name").String())
	...
	for _, modelSpecific := range [][]byte{
		[]byte(`{"type":"codex.rate_limits","metered_limit_name":"codex_bengalfox","rate_limits":{"primary":{"used_percent":70}}}`),
		[]byte(`{"type":"codex.rate_limits","limit_name":"codex-bengalfox","rate_limits":{"primary":{"used_percent":70}}}`),
	} {
		preserved, keep := disabledSvc.finalizeCodexClientQuotaEvent(modelSpecific, disabledContext, &Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth})
		require.True(t, keep)
