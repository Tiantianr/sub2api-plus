...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	group := &Group{
		...
		MaxReasoningEffortOverLimit:     NormalizeMaxReasoningEffortOverLimit(input.MaxReasoningEffortOverLimit),
...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	// 限额字段：nil 表示未提供不修改，负数表示无限制，0 表示不允许用量，
	// 正数表示具体限额。
	...
	if input.MaxReasoningEffortOverLimit != nil {
		overLimit, normalizeErr := normalizeMaxReasoningEffortOverLimitForPlatform(group.Platform, *input.MaxReasoningEffortOverLimit)
		if normalizeErr != nil {
