...
```text
上游版本：v0.1.178+custom.001
个人首版：v0.1.178+custom.901
同基线修订：v0.1.178+custom.902
新基线首版：v0.1.179+custom.901
```
...
| --- | --- | --- |
| `v0.1.178+custom.901` | `v0.1.178+custom.001` | 计划发布 |
| `v0.1.178+custom.001` | 同名上游版本 | 个人仓库回退基线镜像 |

## 一键更新边界

本 fork 已将一键更新源切换到 `Tiantianr/sub2api-plus`。个人 Release 尚未通过完整发布验证前，界面可能查不到资产，
因此生产环境仍不得使用“升级”或“版本回退”。

本 fork 的首版发布必须满足以下条件：
...
5. 将 ID3 Compose 固定到同版本的个人不可变镜像，不能只依赖容器可写层中的自更新二进制。
...
在首次二开发布前，应先备份生产数据，并让 Compose 镜像与当前运行版本一致；该调整必须作为单独的生产变更执行，不与首个二开功能混在一起。
