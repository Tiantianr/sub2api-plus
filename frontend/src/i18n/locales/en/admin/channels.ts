export default {
    ...
    riskControl: {
      ...
      textApiModeDesc: {
        auto: 'When Prompt Guard covers the request, text moderation becomes shadow-only; otherwise it still blocks. Images follow the global mode except for Prompt Audit blocking-exempt users, whose images are reviewed asynchronously without enforcement.',
        blocking: 'The Moderation API judges both text and images under the global mode, duplicating Prompt Guard text review. Exempt-user images remain asynchronous and non-enforcing.',
        observe: 'Text API results are comparison-only: no blocking, risk hash, notification, or ban. Images follow the global mode unless the user is Prompt Audit blocking-exempt.',
        off: 'Text does not call the Moderation API. Images follow the global mode unless the user is Prompt Audit blocking-exempt; local keyword and text-hash rules remain active.',
      },
      ...
      inputDetailLoading: 'Loading complete audited content',
      inputDetailHistorical: 'This historical record retained only an input excerpt; its complete content cannot be recovered.',
      inputDetailLoadFailed: 'Complete audited content could not be loaded. The saved input excerpt is shown instead.',
