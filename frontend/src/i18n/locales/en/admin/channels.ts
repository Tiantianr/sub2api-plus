export default {
    ...
    riskControl: {
      ...
      modePreBlockDesc: 'Synchronously reviews ordinary requests and rejects valid findings. Images from Prompt Audit blocking-exempt users are reviewed asynchronously; Moderation API failures are recorded and allowed.',
      ...
      textApiModeDesc: {
        auto: 'When blocking Prompt Guard covers this request, text moderation becomes shadow-only; otherwise it remains blocking. Images follow the global mode, except blocking-exempt requests are reviewed asynchronously without enforcement.',
        blocking: 'The Moderation API judges text under the global mode. Images remain independently moderated; blocking-exempt requests are asynchronous and non-enforcing.',
        observe: 'Text API findings are recorded for comparison only: no blocking, risk hash, notification, violation count, or ban. Images follow the global mode, except blocking-exempt requests.',
        off: 'Text does not call the Moderation API. Images follow the global mode, except blocking-exempt requests; local keyword and hash rules remain active.',
      },
      policyHelp: {
        open: 'View the complete global mode and text API policy matrix',
        title: 'Content moderation policy combinations',
        summary: 'The global mode determines whether Content Moderation runs synchronously or asynchronously and its maximum blocking authority. The text API policy only refines text Moderation API calls and decisions. Images, keywords, and hashes follow their own controls; Prompt Audit blocking exemptions always remain non-enforcing.',
        ...
        combinations: {
          pre_block: {
            ...
            observe: 'Records text asynchronously in shadow mode; images remain synchronously moderated unless this request is blocking-exempt.',
            off: 'Skips text Moderation; images, keywords, and hashes still follow their own controls, with blocking exemptions remaining non-enforcing.',
          },
          observe: {
      ...
      preBlockErrorsHint: 'API failures or no usable key; recorded without blocking the current request',
      ...
      keywordModeKeywordOnlyDesc: 'Use keywords only for text; misses skip the text API. Images follow the global mode, except blocking-exempt images are asynchronous and non-enforcing.',
      keywordModeKeywordOnlyNotice: 'Keyword-only text strategy: unmatched text-only requests pass without the API; images are still moderated under the request-time exemption policy.',
      ...
      blockingExemptAtRequest: 'Non-blocking',
      blockingExemptAtRequestHint: 'This request belongs to a blocking-exempt user and was allowed while recording audit data.',
    },
