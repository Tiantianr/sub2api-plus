export default {
  promptAudit: {
    ...
    actions: { refresh: 'Refresh runtime', retry: 'Retry', Allow: 'Allow', Warn: 'Warn', Block: 'Block', Error: 'Error' },
    common: { actions: 'Actions', never: 'Never' },
    analysis: {
      action: 'Analyze session', title: 'User session analysis', loading: 'Analyzing this session…',
      user: 'User', session: 'Session', records: 'Chat records', model: 'Audit model', report: 'Analysis report',
    },
    ...
    decisions: { pass: 'Pass', flag: 'Flag', critical: 'Critical', failed: 'Failed' },
    ...
    metrics: { total: 'Total', allowed: 'Normal allows', flagged: 'Flagged', blocked: 'Blocked', unavailable: 'Unavailable', failureAllowed: 'Failure allows', timeouts: 'Timeouts', failovers: 'Failovers', extractionAttempted: 'Attempted', extractionSucceeded: 'Succeeded', extractionEmpty: 'No text', extractionFailed: 'Failed' },
    pool: {
      ...
      timeoutRange: 'Supported range: {min}–{max} ms.', inputLimitRange: 'Supported range: {min}–{max} Unicode characters.', inputLimitBehavior: 'The latest input is submitted before history. Synchronous review scans current and historical user receipt misses; async-only current-user input is always reviewed. Oversized prioritized segments are split at the smallest limit among enabled nodes; set the limit no lower than one message to keep that message intact.',
    ...
    policy: {
      ...
      blockingExemptUsers: 'Blocking-exempt users', blockingExemptUsersHint: 'These users are still fully audited and their events are recorded. Risk findings are recorded without blocking the request. Dependency and extraction failures keep the existing security policy.', blockingExemptUserCount: '{count} blocking-exempt users selected, up to 100',
    ...
    retention: {
      title: 'Full Pass evidence', description: 'Every Pass appears as a lightweight event. Unselected Pass chat content is retained for seven days; selected Pass evidence and risk findings retain their chat content indefinitely.',
    ...
    events: {
      title: 'Audit events', description: 'Review retained events by identity, IP, route, risk, hash, and time. Each event can analyze its own user session.', decision: 'Decision', risk: 'Risk level', failureReason: 'Failure reason', failureReasonFallback: 'Audit failure', failureReasons: { prompt_guard_unavailable: 'Guard temporarily unavailable', prompt_guard_invalid_response: 'Guard returned an invalid response', payload_missing: 'Audit payload unavailable', config_version_changed: 'Audit configuration changed', no_enabled_endpoint: 'No Guard node is enabled' }, endpoint: 'Endpoint', clientIp: 'Client IP', filterByIp: 'Filter by IP {ip}', groupId: 'Group ID', userId: 'User ID', filterByUserId: 'Filter by user ID {id}', apiKeyId: 'API Key ID', keyword: 'Keyword',
      ...
      selectAll: 'Select all events on this page', selectEvent: 'Select event {id}', time: 'Time', identity: 'User / email / API Key', user: 'Username', email: 'User email', apiKey: 'API Key name', group: 'Group', route: 'Endpoint / model', auditNode: 'Audit node', auditModel: 'Audit model', result: 'Decision / risk', durations: 'Queue / audit', queueDelay: 'Queue delay', auditLatency: 'Audit latency', blockingExemptAtRequest: 'Non-blocking', blockingExemptAtRequestHint: 'This request skipped synchronous Guard and entered full asynchronous audit.', preview: 'Redacted preview', empty: 'No matching events.',
      passRetentionNotice: 'Unselected Pass chat content is retained for seven days; selected Pass evidence and risk findings retain their chat content indefinitely.', openConfiguration: 'Open configuration',
      ...
      promptFullHint: 'The Guard input is stored in the event session content record for admin review. Complete-context download also includes canonical content excluded from Guard; treat both as sensitive data.',
      passEvidenceNotRetained: 'This Pass keeps only its lightweight redacted summary. Full evidence was not retained for this user.', promptTruncatedWarning: 'This historical or incomplete event was already truncated and cannot be recovered. The page and download contain only the retained portion.',
      ...
      score: 'Score',
      categories: 'Categories', model: 'Model', stage: 'Request stage', executionMode: 'Execution mode', promptSize: 'Audited size', promptSizeValue: '{chars} characters · {messages} messages', contentStatus: 'Content status', contentComplete: 'Complete', contentTruncated: 'Truncated', contentLightweight: 'Lightweight summary', contentExpired: 'Chat content expired', noRisks: 'No derived risk summaries for this event.',
    ...
    cleanup: {
      action: 'Clean normal events', title: 'Clean normal Pass events', description: 'Only Pass events are cleaned. Flag and Critical findings are never selected. A server preview is required before deletion.',
      userScope: 'User scope', allUsers: 'All users', selectedUser: 'Selected user', userRequired: 'Select one user.', matched: '{count} normal events match', contexts: 'Complete conversations', estimatedBytes: 'Estimated backup reduction', estimateHint: 'This estimates stored content. The next logical backup shrinks, but database files may not immediately return space.', previewRequired: 'Generate a server preview before confirming.', confirm: 'Clean normal events',
    },
    messages: { saved: 'Prompt Audit configuration saved; plaintext API Key state was cleared.', retentionSaved: 'Full Pass evidence users saved.', probeSucceeded: 'The audit node is reachable.', deleted: 'Deleted {count} audit events.' },
    errors: {
      loadConfig: 'Unable to load Prompt Audit configuration.', loadRetention: 'Unable to load full Pass evidence settings.', loadRuntime: 'Unable to load Prompt Audit runtime.', loadGroups: 'Unable to load groups.', loadEvents: 'Unable to load audit events.', loadDetail: 'Unable to load event details.', downloadContext: 'Unable to download the complete context.', saveConfig: 'Unable to save the configuration.', saveRetention: 'Unable to save full Pass evidence settings.', probe: 'Node probe failed.', delete: 'Unable to delete events.', previewDelete: 'Unable to create a deletion preview. Check the time range.', deleteConfirmation: 'The deletion confirmation is invalid or expired. Preview again.',
      ...
      prompt_audit_pass_retention_conflict: 'Another administrator updated the retention list. Reload it before saving.',
      prompt_audit_pass_retention_user_limit: 'Full Pass evidence retention is limited to 100 users.',
      prompt_audit_blocking_exempt_user_limit: 'Blocking exemptions are limited to 100 users.',
      prompt_audit_invalid_blocking_exempt_user: 'The blocking-exempt user ID is invalid.',
      ...
      prompt_audit_invalid_client_ip: 'The client IP filter is invalid.', analyzeEvent: 'Unable to analyze this session.',
      prompt_audit_analysis_unavailable: 'The audit analysis service is temporarily unavailable.', prompt_audit_chat_records_not_found: 'No analyzable chat records are available for this session.',
    },
  },
}
