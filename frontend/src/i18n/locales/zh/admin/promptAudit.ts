export default {
  promptAudit: {
    ...
    events: {
      ...
      passRetentionNotice: '正常通过记录默认不保存；当前为 {count} 名用户开启留存。Flag 和 Critical 问题记录始终保存。', openConfiguration: '前往配置',
      ...
      promptTruncatedWarning: '这是一条历史或不完整事件，入库内容已被截断且无法恢复；页面和下载只能提供当前保留部分。',
      guardReturn: '模型审计返回',
      guardReturnHint: '展示 Guard 归一化后的结构化结果（判定、分类、分数与脱敏证据），不含原始响应体。',
      riskSummaries: '风险摘要',
      evidence: '脱敏证据',
      score: '分数',
      categories: '分类', model: '模型', stage: '请求阶段', executionMode: '执行模式', promptSize: '送检规模', promptSizeValue: '{chars} 字符 · {messages} 条消息', contentStatus: '内容状态', contentComplete: '完整', contentTruncated: '已截断', noRisks: '本事件没有派生风险摘要。',
    ...
    messages: { saved: '提示词审计配置已保存，明文 API Key 状态已清除。', retentionSaved: '正常记录留存用户已保存。', probeSucceeded: '审计节点连接正常。', deleted: '已删除 {count} 条审计事件。' },
    errors: {
      loadConfig: '无法加载提示词审计配置。', loadRetention: '无法加载正常记录留存配置。', loadRuntime: '无法加载提示词审计运行态。', loadGroups: '无法加载分组列表。', loadEvents: '无法加载审计事件。', loadDetail: '无法加载事件详情。', downloadContext: '无法下载完整上下文。', saveConfig: '配置保存失败。', saveRetention: '正常记录留存配置保存失败。', probe: '节点探测失败。', delete: '事件删除失败。', previewDelete: '无法生成删除预览，请检查时间范围。', deleteConfirmation: '删除确认无效或已过期，请重新预览。',
      ...
      prompt_audit_pass_retention_user_limit: '正常记录留存用户不能超过 100 人。',
