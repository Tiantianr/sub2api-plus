export default {
  promptAudit: {
    ...
    actions: { refresh: '刷新运行态', retry: '重试', Allow: '放行', Warn: '警告', Block: '阻止', Error: '异常' },
    common: { actions: '操作', never: '从未' },
    analysis: {
      action: '分析会话', title: '用户会话分析', loading: '正在分析该会话…',
      user: '用户', session: '会话', records: '聊天记录', model: '审计模型', report: '分析报告',
    },
    ...
    decisions: { pass: '通过', flag: '标记', critical: '严重', failed: '异常' },
    ...
    metrics: { total: '总计', allowed: '正常放行', flagged: '标记', blocked: '阻止', unavailable: '不可用', failureAllowed: '故障放行', timeouts: '超时', failovers: '故障切换', extractionAttempted: '尝试提取', extractionSucceeded: '提取成功', extractionEmpty: '无文本', extractionFailed: '提取失败' },
    pool: {
    ...
    policy: {
      ...
      blockingExemptUsers: '免阻塞用户', blockingExemptUsersHint: '这些用户仍会完整执行 Prompt Audit 并生成事件；风险命中只记录，不阻止请求。依赖故障和内容提取失败仍遵循现有安全策略。', blockingExemptUserCount: '已选择 {count} 名免阻塞用户，最多 100 名',
    ...
    retention: {
      title: '通过记录完整证据', description: '所有 Pass 都会显示轻量记录。未选中用户的 Pass 聊天内容保留 7 天；选中的 Pass 证据和风险命中内容长期保留。',
    ...
    events: {
      ...
      passRetentionNotice: '未选中用户的 Pass 聊天内容保留 7 天；选中的 Pass 证据和风险命中内容长期保留。', openConfiguration: '前往配置',
      ...
      promptFullHint: '当前展示事件所属会话中的 Guard 实际送检内容；完整上下文下载还包含被 Guard 选段排除的 canonical 内容，请按敏感数据妥善处理。',
      passEvidenceNotRetained: '这条 Pass 仅保留轻量脱敏摘要；该用户未开启完整证据留存。', promptTruncatedWarning: '这是一条历史或不完整事件，入库内容已被截断且无法恢复；页面和下载只能提供当前保留部分。',
      guardReturn: '模型审计返回',
      guardReturnHint: '展示 Guard 归一化后的结构化结果（判定、分类、分数与脱敏证据），不含原始响应体。',
      riskSummaries: '风险摘要',
      evidence: '脱敏证据',
      score: '分数',
      categories: '分类', model: '模型', stage: '请求阶段', executionMode: '执行模式', promptSize: '送检规模', promptSizeValue: '{chars} 字符 · {messages} 条消息', contentStatus: '内容状态', contentComplete: '完整', contentTruncated: '已截断', contentLightweight: '轻量摘要', contentExpired: '聊天内容已过期', noRisks: '本事件没有派生风险摘要。',
    ...
    cleanup: {
      action: '清理正常记录', title: '清理正常通过记录', description: '只清理 Pass 记录，不会删除 Flag 或 Critical 问题记录。必须先获取预览，才能确认删除。',
      userScope: '用户范围', allUsers: '全部用户', selectedUser: '指定用户', userRequired: '请选择一个用户。', matched: '匹配 {count} 条正常记录', contexts: '完整聊天份数', estimatedBytes: '预计后续备份减少', estimateHint: '这是内容存储估算；下一份逻辑备份会缩小，但数据库文件不保证立即返还磁盘。', previewRequired: '请先获取服务端预览。', confirm: '确认清理正常记录',
    },
    messages: { saved: '提示词审计配置已保存，明文 API Key 状态已清除。', retentionSaved: '通过记录完整证据用户已保存。', probeSucceeded: '审计节点连接正常。', deleted: '已删除 {count} 条审计事件。' },
    errors: {
      loadConfig: '无法加载提示词审计配置。', loadRetention: '无法加载通过记录完整证据配置。', loadRuntime: '无法加载提示词审计运行态。', loadGroups: '无法加载分组列表。', loadEvents: '无法加载审计事件。', loadDetail: '无法加载事件详情。', downloadContext: '无法下载完整上下文。', saveConfig: '配置保存失败。', saveRetention: '通过记录完整证据配置保存失败。', probe: '节点探测失败。', delete: '事件删除失败。', previewDelete: '无法生成删除预览，请检查时间范围。', deleteConfirmation: '删除确认无效或已过期，请重新预览。',
      prompt_audit_config_conflict: '配置已被其他管理员更新。请重新加载服务端配置，再决定如何合并本地草稿。',
      prompt_audit_pass_retention_conflict: '留存名单已被其他管理员更新，请重新加载后再保存。',
      prompt_audit_pass_retention_user_limit: 'Pass 完整证据留存用户不能超过 100 人。',
      prompt_audit_blocking_exempt_user_limit: '免阻塞用户不能超过 100 人。',
      prompt_audit_invalid_blocking_exempt_user: '免阻塞用户 ID 无效。',
      ...
      prompt_audit_invalid_client_ip: '客户端 IP 筛选值无效。', analyzeEvent: '无法分析该会话。',
      prompt_audit_analysis_unavailable: '审计分析服务暂时不可用。', prompt_audit_chat_records_not_found: '该会话没有可分析的聊天记录。',
    },
  },
}
