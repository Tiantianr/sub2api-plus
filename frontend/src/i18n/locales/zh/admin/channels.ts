export default {
    ...
    riskControl: {
      ...
      textApiModeDesc: {
        auto: 'Prompt Guard 覆盖当前请求时，文本 API 仅影子记录；未覆盖时仍同步裁决。图片按全局模式审核，但提示词审计免阻塞用户的图片改为异步且不执行处罚。',
        blocking: '文本和图片都由 Moderation API 按全局模式裁决，会与 Prompt Guard 重复审核文本；免阻塞用户的图片仍为异步且不执行处罚。',
        observe: '文本 API 只记录对比结果，不拦截、不写风险 hash、不触发通知或封禁；图片按全局模式裁决，提示词审计免阻塞用户除外。',
        off: '文本不调用 Moderation API；图片按全局模式裁决，提示词审计免阻塞用户除外；本地关键词和文本 hash 规则不受影响。',
      },
      ...
      inputDetailLoading: '正在加载完整审核内容',
      inputDetailHistorical: '该历史记录生成时仅保留了输入摘要，无法恢复完整内容。',
      inputDetailLoadFailed: '完整审核内容加载失败，当前显示已保存的输入摘要。',
