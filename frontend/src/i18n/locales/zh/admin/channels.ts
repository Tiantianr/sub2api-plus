export default {
    ...
    riskControl: {
      ...
      mode: '全局模式',
      modePreBlock: '前置拦截',
      modePreBlockDesc: '普通请求同步审核，有效命中后立即拒绝；提示词审计免阻塞用户的图片改为异步审核，Moderation API 异常只记录并放行。',
      modeObserve: '仅观察',
      ...
      textApiModeDesc: {
        auto: '仅当同步 Prompt Guard 确实覆盖当前请求时，文本 API 改为影子记录；否则仍同步裁决。图片按全局模式审核，但免阻塞请求改为异步且不执行处罚。',
        blocking: '文本由 Moderation API 按全局模式裁决；图片保持独立审核，免阻塞请求改为异步且不执行处罚。',
        observe: '文本 API 只记录对比结果，不拦截、不写风险 Hash、不通知、不累计违规或封禁；图片按全局模式裁决，免阻塞请求除外。',
        off: '文本不调用 Moderation API；图片按全局模式裁决，免阻塞请求除外；本地关键词和 Hash 规则不受影响。',
      },
      policyHelp: {
        open: '查看全局模式与文本 API 策略的完整组合说明',
        title: '内容审核策略组合说明',
        summary: '全局模式决定 Content Moderation 是否运行、同步或异步，以及最高阻断权限；文本 API 策略只细化文本 Moderation API 的调用和裁决方式。图片、关键词与 Hash 仍按各自配置执行，提示词审计免阻塞请求始终不执行处罚。',
        current: '实际行为：',
        globalMode: '全局模式',
        ...
        combinations: {
          pre_block: {
            ...
            observe: '文本异步影子记录；图片仍按前置模式同步审核，免阻塞请求除外。',
            off: '不调用文本 Moderation；图片、关键词和 Hash 仍按各自配置执行，免阻塞请求仍不执行处罚。',
          },
          observe: {
      ...
      preBlockErrors: '审核异常',
      preBlockErrorsHint: 'API 失败或无可用 Key；记录异常但不阻塞当前请求',
      ...
      keywordModeKeywordOnlyDesc: '文本只用关键词判断，未命中即放行且不调用文本 API；图片按全局模式审核，但免阻塞图片异步且不执行处罚。',
      keywordModeKeywordOnlyNotice: '当前为「仅关键词」文本策略：未命中关键词的纯文本请求直接放行；图片按请求时免阻塞策略调用审计 API。',
      ...
      blockingExemptAtRequest: '免阻塞',
      blockingExemptAtRequestHint: '该请求来自免阻塞用户，已放行并在后台记录审计数据。',
    },
