...
const baseConfig = (): PromptAuditConfig => ({
  ...
  worker_count: 4, queue_capacity: 100, scanners: SCANNER_CATALOG.map((item) => item.id), all_groups: true, group_ids: [], blocking_exempt_user_ids: [],
...
describe('PromptAuditView', () => {
  ...
  it('requires confirmation for blocking and disables it when audit is turned off', async () => {
    ...
    expect(wrapper.find('[data-test="failure-allow-toggle"]').exists()).toBe(false)
